...
const MapBuilder: React.FC = () => {
  ...
  const loadAreas = async () => {
    ...
    try {
      ...
      if (routeData && routeData.length > 0) {
        routeData.forEach(({ area_name, route_number }: { area_name: string; route_number: number }) => {
          countByArea.set(area_name, (countByArea.get(area_name) ?? 0) + 1);
          const current = maxRouteByArea.get(area_name) ?? 0;
          if (route_number > current) maxRouteByArea.set(area_name, route_number);
        });

        for (const area of areaData) {
          const maxSaved = maxRouteByArea.get(area.area_name);
          if (maxSaved === undefined) continue;
          const start = area.route_start ?? 1;
          const expectedCount = maxSaved - start + 1;
          if (expectedCount > area.route_count) {
            await supabase
              .from('area_prefixes')
              .update({ route_count: expectedCount })
              .eq('area_name', area.area_name);
            area.route_count = expectedCount;
          }
        }
      }

      setSavedCounts(countByArea);
      setAreas(areaData as AreaPrefix[]);
    } finally {
      setLoadingAreas(false);
    }
  };

  const openNewAreaModal = () => { setModal({ ...EMPTY_MODAL, open: true }); };
  ...
  const handleSaveArea = async () => {
    ...
    try {
      ...
      const newName = areaName.trim();
      const oldName = modal.editing?.area_name;
      ...
      if (isRename) {
        // Cascade rename: update route_maps first, then insert new area row, delete old
        await supabase.from('route_maps').update({ area_name: newName }).eq('area_name', oldName);
        await supabase.from('area_prefixes').insert(record);
        await supabase.from('area_prefixes').delete().eq('area_name', oldName);
      } else {
  ...
  const handleDeleteArea = async (area: AreaPrefix, e: React.MouseEvent) => {
    e.stopPropagation();
    const confirmed = window.confirm(`Delete "${area.area_name}" and all its saved routes? This cannot be undone.`);
    if (!confirmed) return;
    try {
      await supabase.from('route_maps').delete().eq('area_name', area.area_name);
      await supabase.from('area_prefixes').delete().eq('area_name', area.area_name);
      await loadAreas();
  ...
  useEffect(() => {
    ...
    map.on('load', () => {
      ...
      map.addLayer({ id: 'roads-ghost-layer', type: 'line', source: 'roads-ghost', paint: { 'line-color': ['get', 'color'], 'line-width': 4, 'line-opacity': 0 }, layout: { 'line-cap': 'round', 'line-join': 'round' } });
  ...
  useEffect(() => {
    ...
    const restore = async () => {
      try {
        ...
        const routeUpdates = new Map<number, { selectedWayIds: Set<number>; streetNames: string[]; status: 'pending' | 'approved' | 'flagged'; aiNotes: string; confidence: number; }>();
        for (const saved of data) {
          ...
          for (const seg of (saved.segments || [])) {
            ...
            let wayId: number;
            if (realWay && realWay.geometry.length === savedCoords.length &&
              ...
              const synthetic: OsmWay = { id: splitCounterRef.current++, rootId: seg.osmId, name: seg.name || '', unnamed: !seg.name, geometry: savedCoords };
          ...
          if (selectedWayIds.size > 0) {
            routeUpdates.set(saved.route_number, { selectedWayIds, streetNames, status: saved.status as 'pending' | 'approved' | 'flagged', aiNotes: saved.ai_notes || '', confidence: saved.ai_confidence || 0 });
          }
        }
        setRoutes(prev => prev.map(route => { const update = routeUpdates.get(route.num); if (!update) return route; return { ...route, ...update }; }));
        if (syntheticWaysToAdd.length > 0) setAllWays(prev => [...prev, ...syntheticWaysToAdd]);
      } catch { /* silent */ }
    };
    restore();
  }, [allWays, currentArea]);
  ...
  const handleSaveAll = async () => {
    ...
    try {
      ...
      // Update area_prefixes route_count to reflect actual number of routes
      const start = currentArea.route_start ?? 1;
      const maxRouteNum = routes[routes.length - 1]?.num ?? start;
      const newRouteCount = maxRouteNum - start + 1;
      await supabase.from('area_prefixes').update({ route_count: newRouteCount }).eq('area_name', currentArea.area_name);
      // Update local state so card reflects new range immediately on return
      setAreas(prev => prev.map(a => a.area_name === currentArea.area_name ? { ...a, route_count: newRouteCount } : a));
          ...
          {loadingAreas ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
              {areas.map(area => {
                ...
                return (
                  <div key={area.area_name} onClick={() => handleOpenArea(area)} className="relative bg-gray-800 border border-gray-700 rounded-xl p-4 cursor-pointer hover:border-purple-500 hover:bg-gray-750 transition-all group">
                    <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity flex gap-1">
                      <button onClick={e => openEditAreaModal(area, e)} className="text-gray-500 hover:text-white bg-gray-700 hover:bg-gray-600 rounded p-1" title="Edit area"><Edit2 size={11} /></button>
                      <button onClick={e => handleDeleteArea(area, e)} className="text-gray-500 hover:text-red-400 bg-gray-700 hover:bg-gray-600 rounded p-1" title="Delete area"><X size={11} /></button>
                    </div>
      ...
      <div className="flex flex-1 overflow-hidden" style={{ display: view === 'map' ? 'flex' : 'none' }}>
            ...
            <div className="flex-1 overflow-y-auto">
              {routes.map((r) => (
                <React.Fragment key={r.num}>
                  <button onClick={() => setActiveRouteNum(r.num)} className={`w-full flex items-center gap-2 px-3 py-2 text-left border-b border-gray-700/40 hover:bg-gray-700 transition-colors ${r.num === activeRouteNum ? 'bg-gray-700 border-l-2 border-l-blue-400' : ''}`}>
                    ...
                    {r.status !== 'pending' && r.num === activeRouteNum && (
                      <button onClick={e => { e.stopPropagation(); setRoutes(prev => prev.map(rt => rt.num === r.num ? { ...rt, status: 'pending' } : rt)); }} title="Unlock for editing" className="text-gray-500 hover:text-yellow-400 transition-colors flex-shrink-0 ml-1"><Pencil size={9} /></button>
                    )}
                  </button>
                  <div className="group relative flex items-center justify-center h-0 overflow-visible z-10">
                    <button onClick={() => handleInsertRoute(r.num)} title={`Insert new route after ${formatRouteCode(currentArea.prefix, r.num)}`} className="absolute opacity-0 group-hover:opacity-100 transition-opacity bg-blue-700 hover:bg-blue-500 text-white rounded-full w-4 h-4 flex items-center justify-center shadow-lg border border-blue-500" style={{ top: '-8px' }}><Plus size={9} /></button>
                  </div>
                </React.Fragment>
              ))}
              {routes.length > 0 && (
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          <div style={{ flex: 1, position: 'relative', minHeight: 0 }}>
            ...
            {contextMenu && (
              <div style={{ position: 'fixed', left: contextMenu.x, top: contextMenu.y, zIndex: 50 }} className="bg-gray-900 border border-gray-600 rounded-lg shadow-2xl p-3 w-56" onMouseDown={e => e.stopPropagation()}>
                ...
                <input ref={contextMenuInputRef} value={contextMenuInput} onChange={e => setContextMenuInput(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') handleRenameConfirm(); if (e.key === 'Escape') setContextMenu(null); }} placeholder="e.g. Private Road" className="w-full bg-gray-800 border border-gray-600 rounded px-2 py-1.5 text-xs text-white focus:outline-none focus:border-blue-500 mb-2" />
                <div className="flex gap-1.5">
                  <button onClick={handleRenameConfirm} disabled={!contextMenuInput.trim()} className="flex-1 bg-blue-700 hover:bg-blue-600 disabled:opacity-40 text-white text-xs py-1.5 rounded flex items-center justify-center gap-1"><Check size={11} />Apply</button>
          ...
          {currentArea && (
            <div className="bg-gray-800 border-t border-gray-700 px-4 py-2 flex items-center gap-3 flex-shrink-0">
              ...
              <button onClick={() => setShowGhostRoutes(v => !v)} className={`flex items-center gap-2 px-3 py-1.5 rounded text-sm border transition-colors ${showGhostRoutes ? 'bg-indigo-900/40 border-indigo-500 text-indigo-300' : 'bg-gray-700 border-gray-600 text-gray-400 hover:bg-gray-600'}`}>
                {showGhostRoutes ? <Eye size={14} /> : <EyeOff size={14} />}
                {showGhostRoutes ? 'Routes On' : 'Routes Off'}
              </button>
              <button onClick={handleApprove} disabled={!activeRoute || activeRoute.selectedWayIds.size === 0} className="bg-green-800/50 hover:bg-green-700/50 disabled:opacity-50 text-green-400 border border-green-700/50 px-3 py-1.5 rounded text-sm flex items-center gap-2"><Check size={14} />Approve</button>
              <button onClick={handleFlag} className="bg-red-900/30 hover:bg-red-900/50 text-red-400 border border-red-800/50 px-3 py-1.5 rounded text-sm flex items-center gap-2"><X size={14} />Flag</button>
          ...
          <div className="w-64 bg-gray-800 border-l border-gray-700 flex flex-col overflow-hidden flex-shrink-0">
            ...
            <div className="flex-1 overflow-y-auto p-2">
              {!activeRoute?.streetNames.length ? (
                <div className="text-xs text-gray-600 italic text-center mt-6 px-2">No streets selected yet.<br /><br />Shift+drag to box-select, click to toggle, right-click any segment to name it.</div>
              ) : (
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
          <div className="bg-gray-900 rounded-xl border border-gray-700 w-full max-w-md shadow-2xl">
            <div className="p-4 border-b border-gray-700 flex items-center justify-between">
              <h2 className="text-base font-bold text-white flex items-center gap-2"><MapIcon size={16} className="text-purple-400" />{modal.editing ? 'Edit Area' : 'New Area'}</h2>
            ...
            <div className="p-5 space-y-4">
              <div>
                ...
                <input value={modal.areaName} onChange={e => setModal(m => ({ ...m, areaName: e.target.value }))} placeholder="e.g. BURLINGTON SOUTH 2" className="w-full bg-gray-800 border border-gray-600 rounded px-3 py-2 text-white text-sm focus:outline-none focus:border-purple-500" />
              </div>
              <div>
                <label className="block text-xs text-gray-400 mb-1 uppercase tracking-wider">Route Prefix</label>
                <input value={modal.prefix} onChange={e => setModal(m => ({ ...m, prefix: e.target.value.toUpperCase().replace(/[^A-Z]/g, '') }))} placeholder="e.g. BS" maxLength={6} className="w-full bg-gray-800 border border-gray-600 rounded px-3 py-2 text-white text-sm font-mono uppercase focus:outline-none focus:border-purple-500" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs text-gray-400 mb-1 uppercase tracking-wider">Route Start</label>
                  <input type="number" min={1} value={modal.routeStart} onChange={e => setModal(m => ({ ...m, routeStart: Math.max(1, parseInt(e.target.value) || 1) }))} className="w-full bg-gray-800 border border-gray-600 rounded px-3 py-2 text-white text-sm focus:outline-none focus:border-purple-500" />
                </div>
                <div>
                  <label className="block text-xs text-gray-400 mb-1 uppercase tracking-wider">Route End</label>
                  <input type="number" min={modal.routeStart} value={modal.routeEnd} onChange={e => setModal(m => ({ ...m, routeEnd: Math.max(m.routeStart, parseInt(e.target.value) || m.routeStart) }))} className="w-full bg-gray-800 border border-gray-600 rounded px-3 py-2 text-white text-sm focus:outline-none focus:border-purple-500" />
                </div>
              </div>
            ...
            <div className="p-4 border-t border-gray-700 flex justify-end gap-3">
              ...
              <button onClick={handleSaveArea} disabled={savingArea || !modal.areaName.trim() || !modal.prefix.trim() || modalRouteCount === 0} className="bg-purple-600 hover:bg-purple-500 disabled:opacity-50 text-white px-5 py-2 rounded font-medium text-sm flex items-center gap-2">
