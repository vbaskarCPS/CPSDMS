...
const JobDetail: React.FC = () => {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      // Fetch all assignments (both pending and completed) to find this specific job
  ...
  const handleSave = async () => {
    ...
    const tx: SessionTransaction = {
      ...
      routeManagerName: 'RM', // Could fetch proper RM name if needed
    ...
    try {
      ...
      // 3. Update Local Stats (Optimistic UI or fetch fresh)
  ...
  const handleCancel = (status: 'next_time' | 'cancelled') => {
    alert(
      'Status update logic needs to be connected to Supabase update query. (Pending Implementation)'
    );
    navigate('/logsheet');
  };
  ...
  if (loading)
    return (
      <div className="h-screen bg-black flex items-center justify-center">
        <Loader className="text-cps-blue animate-spin" />
      </div>
    );
    ...
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4 animate-fade-in">
      <div className="bg-gray-800 rounded-lg w-full max-w-3xl max-h-[95vh] flex flex-col border border-gray-700 shadow-2xl">
        ...
        <div className="flex justify-between items-center p-4 border-b border-gray-700 bg-gray-900/50 rounded-t-lg flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            {isReadOnly && (
              <span className="bg-blue-900/30 text-blue-300 text-xs px-2 py-0.5 rounded border border-blue-800 flex items-center gap-1">
        ...
        <div className="overflow-y-auto p-4 space-y-6 flex-grow custom-scrollbar">
          {/* FORM CONTROLS (Identical to before, omitting verbose jsx for brevity but ensuring key bindings work) */}
          {/* CLIENT DETAILS */}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="md:col-span-1">
                ...
                <input
                  value={routeNumber}
                  disabled
                  className="w-full bg-gray-800 border border-gray-700 rounded p-2 text-white font-mono opacity-50 cursor-not-allowed"
                />
              </div>
              ...
              {/* ... (Address fields same as original) ... */}
          ...
          {/* SERVICES & PAYMENT (Same as original but with updated handlers) */}
          <div
          ...
          >
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className={`flex-grow bg-gray-800 border border-gray-700 rounded p-2 text-xl font-mono font-bold text-green-400 outline-none ${
                  ...
                  {!isPrepaid && !isReadOnly && (
                    <button
                      ...
                      className="px-3 bg-gray-700 text-gray-300 rounded border border-gray-600 hover:bg-gray-600 font-bold text-xs"
                    >
                      + Tax
                    </button>
                  )}
            ...
            {paymentMethod === 'Credit Card' && (
              <div
                className={`mt-2 p-3 rounded border flex items-center justify-between ${
                  isCreditPaid
              ...
              >
                <span className="text-sm font-medium">
                  {isCreditPaid ? 'Card Processed' : 'Payment Required'}
                </span>
                {isCreditPaid ? (
                  <CheckCircle size={20} />
                ) : (
          ...
          {!isReadOnly ? (
            <>
              <button
                ...
                className="flex items-center gap-2 px-4 py-3 bg-red-900/20 hover:bg-red-900/40 text-red-300 border border-red-800 rounded-md font-bold transition-colors"
              >
                <Ban size={18} /> Cancel / Skip
              </button>
              <button
                onClick={handleSave}
                disabled={paymentMethod === 'Credit Card' && !isCreditPaid}
            ...
            <p className="text-center w-full text-gray-500 italic text-sm">
              Record finalized.
            </p>
          )}
        </div>
      ...
      {showCancelModal && (
        <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-800 rounded-lg p-6 w-full max-w-sm text-center border border-gray-700">
            ...
            <div className="space-y-3">
              <button
                ...
                className="w-full py-3 bg-yellow-600/20 text-yellow-400 border border-yellow-700/50 rounded font-bold"
              >
                Next Time
              </button>
              <button
                onClick={() => handleCancel('cancelled')}
                className="w-full py-3 bg-red-600/20 text-red-400 border border-red-700/50 rounded font-bold"
              >
                Cancelled
              </button>
              <button
