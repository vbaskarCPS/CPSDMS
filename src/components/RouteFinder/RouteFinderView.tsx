...
import {
  ArrowLeft, MapPin, CheckCircle, SkipForward, AlertCircle,
  Loader, Search, ChevronDown, ChevronUp, RefreshCw,
  Layers, Zap, Users, XCircle,
} from 'lucide-react';
...
const RouteFinderView: React.FC<Props> = ({ onBack }) => {
  ...
  // ── Check auth on mount ────────────────────────────────────────────────────
  useEffect(() => {
    if (routeFinderSheetsService.isAuthenticated()) {
      setPhase(spreadsheetId ? 'setup' : 'setup');
    } else {
      setPhase('auth');
    }
  }, []);
  ...
  const handleStartScan = async () => {
    ...
    try {
      ...
      // Check for Listings tab
      const hasListings = await routeFinderSheetsService.hasListingsTab(resolvedId);
      if (!hasListings) {
        setError('No "Listings" tab found. Please add the East Listings CSV as a tab named "Listings" in your spreadsheet first.');
        setPhase('setup');
        return;
      }
      ...
      // Read all sheets with progress
      const loadedSheets = await routeFinderSheetsService.readCallBookSheets(
        resolvedId,
        tabNames,
        (current, total, sheetName) => setScanProgress({ current, total, sheet: sheetName })
      );
      ...
      if (existingSession) {
        // Filter to only pending rows from the saved session
        const pendingSet = new Set(existingSession.pendingRowIds);
        finalQueue = result.queue.filter(r => pendingSet.has(r.id));
        activeSession = existingSession;
      } else {
        // New session
        finalQueue = result.queue;
        activeSession = await routeFinderSessionService.createSession(
          resolvedId, result.queue, result.totalScanned
        );
      }
  ...
  // ── Get current edit values for a row ─────────────────────────────────────
  ...
  const handleAccept = useCallback(async (row: RouteFinderRow) => {
    ...
    try {
      ...
      // 2. Determine if this is a new street we need to learn
      const isNewStreet = !session.learnedStreets[finalRoute.toUpperCase()]?.includes(
        normalizeStreetForMatch(finalStreet)
      ) && !listingsData.routeMap.get(finalRoute.toUpperCase())?.includes(
        normalizeStreetForMatch(finalStreet)
      );
      ...
      if (isNewStreet && finalStreet && finalRoute) {
        ...
        const result = await routeFinderSessionService.addLearnedStreet({
          ...
          routeCode:              finalRoute,
          originalStreet:         finalStreet,
          currentLearned:         updatedLearned,
          currentLearnedOriginal: updatedLearnedOriginal,
        });
        updatedLearned         = result.learned;
        updatedLearnedOriginal = result.learnedOriginal;

        // Update listings data in memory so cascade check works immediately
      ...
      const { newPendingIds, newFixedRows } = await routeFinderSessionService.markRowFixed({
        sessionId: session.id, rowId: row.id, logEntry,
        currentPendingIds: session.pendingRowIds,
        currentFixedRows: session.fixedRows,
        cascadeResolvedIds: cascadeIds,
      });
      ...
      setSession(prev => prev ? {
        ...prev,
        pendingRowIds: newPendingIds,
        fixedRows: newFixedRows,
        learnedStreets: updatedLearned,
        learnedStreetsOriginal: updatedLearnedOriginal,
      } : null);
      ...
      // Check if complete
      if (newPendingIds.length === 0) setPhase('complete');
  ...
  // ── Skip a row ────────────────────────────────────────────────────────────
  const handleSkip = useCallback(async (row: RouteFinderRow) => {
    if (!session) return;
    const newPendingIds = await routeFinderSessionService.skipRow(
      session.id, row.id, session.pendingRowIds
    );
    setSession(prev => prev ? { ...prev, pendingRowIds: newPendingIds } : null);
    // Move to end of queue visually
    setQueue(prev => {
      const without = prev.filter(r => r.id !== row.id);
      const skipped = prev.find(r => r.id === row.id);
      return skipped ? [...without, { ...skipped, status: 'skipped' }] : without;
    });
  }, [session]);

  // ── Bulk accept orange in current view ────────────────────────────────────
  const handleBulkAcceptOrange = async () => {
    const orangeRows = filteredQueue.filter(r => r.color === 'orange');
    if (orangeRows.length === 0) return;
    if (!window.confirm(`Accept all ${orangeRows.length} orange (typo) rows? This will write directly to your spreadsheet.`)) return;
  ...
  // ── Bulk accept cluster-confirmed ─────────────────────────────────────────
  const handleBulkAcceptCluster = async () => {
    const clusterRows = filteredQueue.filter(r => r.candidates[0]?.isClusterPrimary);
    if (clusterRows.length === 0) return;
    if (!window.confirm(`Accept all ${clusterRows.length} cluster-confirmed rows?`)) return;
    setBulkApplying(true);
  ...
  const handleCandidateSelect = (rowId: string, idx: number, candidate: CandidateRoute) => {
    ...
    setEditedValues(prev => ({
      ...prev,
      [rowId]: { routeCode: candidate.routeCode, streetName: candidate.streetName },
    }));
  };

  // ── Filtered queue ────────────────────────────────────────────────────────
  const filteredQueue = useMemo(() => {
    let rows = queue.filter(r => r.status !== 'fixed');
  ...
  const counters = useMemo(() => {
    ...
    return {
      phone_group: pending.filter(r => r.color === 'phone_group').length,
      orange:      pending.filter(r => r.color === 'orange').length,
      yellow:      pending.filter(r => r.color === 'yellow').length,
      red:         pending.filter(r => r.color === 'red').length,
    ...
    <div className="flex flex-col items-center justify-center min-h-[400px] gap-6 max-w-lg mx-auto">
      ...
      <div className="w-full space-y-3">
        ...
        <button
          ...
          disabled={!spreadsheetInput.trim()}
        ...
        >
          <Search size={18} />
          {spreadsheetId === (spreadsheetInput.match(/\/d\/([a-zA-Z0-9-_]+)/)?.[1] || spreadsheetInput.trim())
            ? 'Resume Session' : 'Start Scan'}
        </button>
      </div>
    </div>
  );
    ...
    <div className="flex flex-col items-center justify-center min-h-[400px] gap-6">
      ...
      <div className="text-center">
        <p className="text-white font-medium">
          {scanProgress.sheet || 'Preparing…'}
        </p>
        {scanProgress.total > 0 && (
          <p className="text-gray-400 text-sm mt-1">
            Sheet {scanProgress.current} of {scanProgress.total}
          </p>
        )}
      </div>
      {scanProgress.total > 0 && (
    ...
    <div className="flex flex-col items-center justify-center min-h-[400px] gap-6">
      ...
      <div className="text-center">
        ...
        <p className="text-gray-400">
          {session?.fixedRows || 0} rows fixed. Your call book is clean.
        </p>
      </div>
      <button
        onClick={() => { setPhase('setup'); setSpreadsheetInput(''); }}
  ...
  const renderWorking = () => {
    const pendingTotal = session?.pendingRowIds.length || 0;
      ...
      <div className="flex h-full gap-0">
        ...
        <div className="flex-1 flex flex-col overflow-hidden min-w-0">
          ...
          <div className="flex items-center gap-2 px-4 py-3 border-b border-gray-700 bg-gray-800/50 flex-wrap">
            <div className="relative flex-shrink-0">
              ...
              <input
                type="text"
                placeholder="Search booking, street, route…"
                value={searchTerm}
            ...
            {(['all', 'phone_group', 'orange', 'yellow', 'red'] as ColorFilter[]).map(f => {
              const count = f === 'all' ? counters.total : counters[f];
            ...
            {filteredQueue.length === 0 ? (
              ...
              filteredQueue.map(row => (
                <RouteRow
                  ...
                  onToggleExpand={() => setExpandedCandidates(prev => {
        ...
        <div className="w-60 border-l border-gray-700 bg-gray-800/30 flex flex-col p-4 gap-5 overflow-y-auto flex-shrink-0">
            ...
            {([['phone_group','🟢',counters.phone_group,'emerald'], ['orange','🟠',counters.orange,'orange'], ['yellow','🟡',counters.yellow,'yellow'], ['red','🔴',counters.red,'red']] as const).map(
              ([color, emoji, count, hue]) => (
                <div key={color} className="flex items-center justify-between">
                  <span className="text-xs text-gray-400">{emoji} {COLOR_CONFIG[color].label.replace(/^[^ ]+ /, '')}</span>
          ...
          <div className="space-y-2 pt-2 border-t border-gray-700">
            ...
            <button
              onClick={handleBulkAcceptOrange}
              disabled={bulkApplying || counters.orange === 0}
            ...
            <button
              onClick={handleBulkAcceptCluster}
  ...
  return (
    <div className="flex flex-col h-full bg-gray-900 text-white relative">
        ...
        <div className="flex items-center gap-3">
          ...
          {phase === 'working' && spreadsheetId && (
            <span className="text-xs text-gray-500 font-mono truncate max-w-[200px]">
              {spreadsheetId.slice(0, 20)}…
            </span>
          )}
        </div>
        {phase === 'working' && (
          <div className="text-xs text-gray-500">
            {counters.total} left · {session?.fixedRows || 0} fixed
          </div>
        )}
      </div>
      ...
      {/* Toast */}
      {toast && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-gray-800 border border-gray-600 text-white text-sm px-5 py-2.5 rounded-full shadow-xl flex items-center gap-2 z-50 animate-fade-in">
...
const RouteRow: React.FC<RouteRowProps> = ({
  ...
  onRouteCodeChange, onStreetNameChange, onCandidateSelect,
  onToggleExpand, onAccept, onSkip,
}) => {
  ...
  return (
    <div className={`${cc.border} ${cc.bg} border-b border-gray-800/60 transition-colors`}>
      <div
        className="grid items-center px-4 py-2.5 gap-2 text-sm"
        style={{ gridTemplateColumns: '5px 80px 90px 1fr auto 1fr 180px 100px' }}
      >
        {/* Color dot */}
        <div />
        ...
        <div className="min-w-0">
          <div className="flex items-center gap-1">
            <span className={`text-xs px-1.5 py-0.5 rounded border font-mono ${cc.badge}`}>
              {row.currentRouteCode || '—'}
            </span>
            {row.isORSuffix && (
              <span className="text-xs bg-purple-900/30 text-purple-400 border border-purple-700 px-1 rounded">OR</span>
            )}
          </div>
          <div className="text-gray-300 text-xs truncate mt-0.5" title={row.currentStreetName}>
        ...
        <div className="min-w-0 space-y-1">
          {/* Route code — dropdown for yellow/phone_group with multiple candidates, text input otherwise */}
            ...
            <div>
              <div className="flex flex-wrap gap-1">
                {top3.map((c, idx) => (
                  <button
                    key={`${c.routeCode}-${idx}`}
                    onClick={() => onCandidateSelect(idx, c)}
                  ...
                  >
                    {c.isClusterPrimary && <span className="mr-1">⚡</span>}
                    {c.routeCode}
                  </button>
                ))}
                {hasMore && (
              ...
              {isExpanded && row.candidates.slice(3).map((c, i) => (
                <button
                  key={`extra-${i}`}
                  onClick={() => onCandidateSelect(i + 3, c)}
        ...
        <div className="text-xs text-gray-500 truncate space-y-0.5" title={(row.clusterSignal || row.phoneGroupSignal)}>
          {row.phoneGroupSignal && (
            <div className="truncate text-emerald-500/80">{row.phoneGroupSignal}</div>
          )}
          {row.clusterSignal && (
            <div className="truncate text-blue-500/80">{row.clusterSignal}</div>
          )}
          {!row.phoneGroupSignal && !row.clusterSignal && (
