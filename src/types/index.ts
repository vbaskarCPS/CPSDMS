...
//
// A RouteSplit divides ONE route into two visual halves ('a' and 'b') purely
// at the RM-tooling layer. The bookings and routes tables keep the same
// route_code — workers continue to see "BIN09" on their logsheet. The RM
// sees two halves "BIN09a" and "BIN09b" on the master map, and per-half
// assignments restrict which bookings each worker actually receives.
//
// Eligibility: route must be unassigned at split time and not already split.
// Persistence: per (CC, session_date, route_code), wiped on session end.
//
// Bucketing semantics:
//   - segmentBOsmIds:  list of OSM segment ids whose midpoint fell inside
//     one of the RM's dragged rectangles. Segments NOT in this list are 'a'.
//   - bookingBIds:     list of booking IDs whose closest segment is in
//     segmentBOsmIds. Bookings NOT in this list are 'a'.
//   - Both are computed ONCE at Confirm time and cached.
//
// The route's existing assigned_worker_ids field continues to be the UNION
// of (assignedWorkersA + assignedWorkersB) so existing per-booking assignment
...
export interface RouteSplit {
  ...
  commandCenterId: string;
  sessionDate: string;               // YYYY-MM-DD
  routeCode: string;                 // the BASE route code (e.g. "BIN09")
  segmentBOsmIds: number[];          // OSM segment ids in the 'b' bucket
  bookingBIds: string[];             // Booking IDs in the 'b' bucket
  assignedWorkersA: string[];        // Worker IDs assigned to half 'a'
  assignedWorkersB: string[];        // Worker IDs assigned to half 'b'
  createdAt?: string;
  updatedAt?: string;
}
