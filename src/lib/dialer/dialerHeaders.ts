...
export interface ColumnIndices {
  ...
  // --- BC-specific: Service flag columns (fixed positions, READ-ONLY) ---
  SVC_A: number;       // Aeration flag (col M = index 12)
  SVC_D: number;       // Dethatch flag (col N = index 13)
  SVC_F: number;       // Fertilizer flag (col O = index 14)
  SVC_S: number;       // Seed flag (col P = index 15)
  SVC_L: number;       // Lime flag (col Q = index 16)

  // --- BC-specific: Upsell columns (matched by header name) ---
  DETHATCH: number;     // DE-THATCH (upsell sold flag)
  DETHATCH_PP: number;  // PP.1 (dethatch prepaid flag)
  DETHATCH_AMT: number; // AMT.1 (dethatch amount)
  REJUV: number;        // REJUV+ (rejuv upsell sold flag)
  REJUV_AMT: number;    // AMT.2 (rejuv amount)
}
...
function defaultCI(): ColumnIndices {
  return {
    ...
    SERVICE_YEARS: [],
    // BC-specific (default off)
    SVC_A: -1, SVC_D: -1, SVC_F: -1, SVC_S: -1, SVC_L: -1,
    DETHATCH: -1, DETHATCH_PP: -1, DETHATCH_AMT: -1,
    REJUV: -1, REJUV_AMT: -1,
  };
}
...
export function resolveHeaders(headers: any[]): ColumnIndices {
  ...
  for (let i = 0; i < headers.length; i++) {
    ...
    if (h === 'PHONE') {
      ...
      CI.BOOKING_NOTES = i;
    // --- BC upsell columns (matched by header name) ---
    } else if ((h === 'DE-THATCH' || h === 'DETHATCH' || h === 'DE_THATCH') && CI.DETHATCH < 0) {
      CI.DETHATCH = i;
    } else if (h === 'PP.1' && CI.DETHATCH_PP < 0) {
      CI.DETHATCH_PP = i;
    } else if (h === 'AMT.1' && CI.DETHATCH_AMT < 0) {
      CI.DETHATCH_AMT = i;
    } else if ((h === 'REJUV+' || h === 'REJUV' || h === 'REJUV +') && CI.REJUV < 0) {
      CI.REJUV = i;
    } else if (h === 'AMT.2' && CI.REJUV_AMT < 0) {
      CI.REJUV_AMT = i;
    } else if (/^\d{4}$/.test(h)) {
...
/**
 * Apply BC fixed-position service flag columns.
 * Called AFTER resolveHeaders when the campaign type is known to be 'bc'.
 * Sets SVC_A through SVC_L to their fixed column positions (12-16).
 * Mutates the CI object in place and returns it for convenience.
 */
export function applyBCFixedColumns(CI: ColumnIndices): ColumnIndices {
  CI.SVC_A = 12;  // Column M = Aeration
  CI.SVC_D = 13;  // Column N = Dethatch
  CI.SVC_F = 14;  // Column O = Fertilizer
  CI.SVC_S = 15;  // Column P = Seed
  CI.SVC_L = 16;  // Column Q = Lime
  return CI;
}
