...
async function renderRouteMapOffscreen(
  ...
  return new Promise<string | null>((resolve) => {
    ...
    container.style.position = 'fixed'; container.style.top = '0px'; container.style.left = '0px';
    container.style.width = `${pixelWidth}px`; container.style.height = `${pixelHeight}px`;
    container.style.zIndex = '-1'; container.style.pointerEvents = 'none'; container.style.overflow = 'hidden';
    ...
    const map = new mapboxgl.Map({
      container, style: 'mapbox://styles/mapbox/streets-v12', center: allCoords[0],
      zoom: 14, preserveDrawingBuffer: true, attributionControl: false, interactive: false,
    });

    const timeout = setTimeout(() => {
      console.warn('[DMB PCL] Map render timed out after 30s');
      try { map.triggerRepaint();
        setTimeout(() => { try { finish(map, map.getCanvas().toDataURL('image/png')); } catch { finish(map, null); } }, 200);
      } catch { finish(map, null); }
    }, 30000);

    map.on('load', () => {
      applyMapStyling(map);
      const routeInsertBefore = (map.getLayer('road-label') ? 'road-label' :
        map.getStyle().layers?.find((l: any) => l.type === 'symbol')?.id) ?? undefined;
      const features: GeoJSON.Feature[] = segments.map((seg) => ({
        type: 'Feature', properties: {}, geometry: { type: 'LineString', coordinates: seg.coordinates },
      }));
      map.addSource('pcl-route-src', { type: 'geojson', data: { type: 'FeatureCollection', features } });
      map.addLayer({
        ...
        setTimeout(() => {
          ...
          map.once('render', () => {
            setTimeout(() => {
              ...
              catch (err) { console.error('[DMB PCL] toDataURL failed:', err); finish(map, null); }
            }, 300);
          });
        }, 2000);
      });
...
function drawBlockBorder(doc: jsPDF, side: 'left' | 'right'): void {
  ...
  doc.setDrawColor(0, 0, 0); doc.setLineWidth(BORDER_W); doc.rect(x, MARGIN, BLOCK_W, BLOCK_H);
}

function drawMapBlock(doc: jsPDF, areaName: string, routeCode: string, mapImage: string | null): void {
  const x = BLOCK_L_X;
  drawBlockBorder(doc, 'left');
  const titleY = MARGIN + BORDER_W; const titleW = BLOCK_W - 2 * BORDER_W;
  doc.setFillColor(255, 255, 255); doc.rect(x + BORDER_W, titleY, titleW, TITLE_H, 'F');
  doc.setDrawColor(0, 0, 0); doc.setLineWidth(BORDER_W);
  ...
  const imgX = x + BORDER_W; const imgY = titleY + TITLE_H;
  const imgW = titleW; const imgH = BLOCK_H - 2 * BORDER_W - TITLE_H;
  if (mapImage) {
    try { doc.addImage(mapImage, 'PNG', imgX, imgY, imgW, imgH); }
    catch { drawMapPlaceholder(doc, imgX, imgY, imgW, imgH, x); }
  } else { drawMapPlaceholder(doc, imgX, imgY, imgW, imgH, x); }
...
function drawMapPlaceholder(doc: jsPDF, imgX: number, imgY: number, imgW: number, imgH: number, blockX: number): void {
  doc.setFillColor(230, 230, 230); doc.rect(imgX, imgY, imgW, imgH, 'F');
...
function truncate(text: string, maxWidthPt: number, fontSize: number): string {
  const glyphW = fontSize * 0.52; const maxChars = Math.floor(maxWidthPt / glyphW);
...
function drawClientGroup(doc: jsPDF, group: ClientGroup, blockX: number, y: number): void {
  ...
  doc.setFillColor(224, 224, 224); doc.setDrawColor(0, 0, 0); doc.setLineWidth(0.5);
  ...
  doc.setFont('helvetica', 'bold'); doc.setFontSize(6.5); doc.setTextColor(17, 17, 17);
  ...
  doc.setFont('helvetica', 'normal'); doc.setFontSize(6); doc.setTextColor(51, 51, 51);
  let rowY = y + CLIENT_HEADER_H;
  for (const hist of group.history) {
    doc.setDrawColor(180, 180, 180); doc.setLineWidth(0.3);
    doc.rect(innerX, rowY, contentW, HISTORY_ROW_H);
    const tY = rowY + HISTORY_ROW_H * 0.72; let hx = innerX + 3;
...
export async function generateDmbPCL(
  ...
  const mapPixelW = 500; const mapPixelH = 730;
  for (let ri = 0; ri < routeDataList.length; ri++) {
    ...
    let side: 'left' | 'right' = 'right'; let curY = contentTop(); let ci = 0;
    while (ci < rd.clients.length) {
      const client = rd.clients[ci]; const groupH = clientGroupHeight(client);
      if (curY + groupH > contentBottom() && curY > contentTop() + 10) {
        if (side === 'right') { doc.addPage(); drawBlockBorder(doc, 'left'); drawBlockBorder(doc, 'right'); side = 'left'; }
        else { side = 'right'; } curY = contentTop();
      }
...
export interface MastermapBookingInput { lat: number; lng: number; routeColor: string; }

export interface MastermapResult {
  success: boolean; routeCount: number; bookingCount: number; errorMessage?: string;
}

interface LegendEntry { number: number; streetName: string; }
interface MastermapRenderResult { imageDataUrl: string; legendEntries: LegendEntry[]; }
interface RoadPath { name: string; roadClass: string; pixelPath: { x: number; y: number }[]; pathLength: number; }
...
function getMastermapDimensions(format: MastermapPageFormat) {
  switch (format) {
    case 'letter-portrait':  return { pageW: 612,  pageH: 792,  sidebarH: 85, orientation: 'portrait'  as const, paper: 'letter' as const };
    case 'letter-landscape': return { pageW: 792,  pageH: 612,  sidebarH: 60, orientation: 'landscape' as const, paper: 'letter' as const };
    case 'legal-portrait':   return { pageW: 612,  pageH: 1008, sidebarH: 85, orientation: 'portrait'  as const, paper: 'legal'  as const };
    case 'legal-landscape':  return { pageW: 1008, pageH: 612,  sidebarH: 60, orientation: 'landscape' as const, paper: 'legal'  as const };
  }
}

// ─── OUTLIER COORDINATE FILTER ───────────────────────────────────────────────

function filterOutlierCoords(coords: [number, number][]): [number, number][] {
  ...
  const cosLat = Math.cos(medLat * Math.PI / 180);
  const dists = coords.map(c => {
    const dx = (c[0] - medLng) * cosLat;
    const dy = c[1] - medLat;
    return Math.sqrt(dx * dx + dy * dy);
  });
  const sortedDists = [...dists].sort((a, b) => a - b);
  const p90 = sortedDists[Math.floor(sortedDists.length * 0.9)];
  const threshold = Math.max(p90 * 3, 0.005);
  const filtered = coords.filter((_, i) => dists[i] <= threshold);
  if (filtered.length < 3) return coords;
  if (filtered.length < coords.length) {
    console.log(`[DMB Mastermap] Outlier filter: removed ${coords.length - filtered.length} of ${coords.length} coordinates`);
  }
  return filtered;
}
...
function applyGreyscaleBase(map: mapboxgl.Map): void {
  map.getStyle().layers?.forEach((layer: any) => {
    ...
    try {
      ...
      } else if (layer.type === 'background') { map.setPaintProperty(layer.id, 'background-color', '#f5f5f5'); }
    } catch { /* skip */ }
  });
}
...
function thickenMastermapRoads(map: mapboxgl.Map): void {
  map.getStyle().layers?.forEach((layer: any) => {
    ...
    try {
      if (id.includes('motorway') || id.includes('trunk')) map.setPaintProperty(layer.id, 'line-width', 44);
      else if (id.includes('primary') || id.includes('secondary')) map.setPaintProperty(layer.id, 'line-width', 32);
      else if (id.includes('street') || id.includes('tertiary') || id.includes('minor') || id.includes('road')) map.setPaintProperty(layer.id, 'line-width', 22);
    } catch { /* skip */ }
  });
}
...
function hideMapboxLabels(map: mapboxgl.Map): void {
  map.getStyle().layers?.forEach((layer: any) => {
    if (layer.type !== 'symbol') return;
    if (layer.id.startsWith('mm-')) return;
...
function detectHighwayBearing(map: mapboxgl.Map): number {
  ...
  map.getStyle().layers?.forEach((layer: any) => {
    ...
    if ((id.includes('motorway') || id.includes('trunk')) && !id.includes('link') && !id.includes('case')) hwLayerIds.push(layer.id);
  });
  ...
  for (const f of features) {
    ...
    else if (geom.type === 'MultiLineString') { for (const c of geom.coordinates) coords.push(...c); }
    for (let i = 1; i < coords.length; i++) {
      const midLat = (coords[i][1] + coords[i - 1][1]) / 2;
      const cl = Math.cos(midLat * Math.PI / 180);
      const dx = (coords[i][0] - coords[i - 1][0]) * cl;
      const dy = coords[i][1] - coords[i - 1][1];
      const len = Math.sqrt(dx * dx + dy * dy);
      if (len < 0.00001) continue;
      const gb = Math.atan2(dx, dy);
      sumSin += Math.sin(2 * gb) * len; sumCos += Math.cos(2 * gb) * len;
    }
  }
  if (Math.abs(sumSin) < 0.0001 && Math.abs(sumCos) < 0.0001) return 0;
  const avg = (Math.atan2(sumSin, sumCos) / 2) * (180 / Math.PI);
  let bH = avg - 90, bV = avg;
  while (bH > 90) bH -= 180; while (bH < -90) bH += 180;
  while (bV > 90) bV -= 180; while (bV < -90) bV += 180;
  return Math.abs(bH) < Math.abs(bV) ? bH : bV;
}
...
class OccupancyGrid {
  ...
  isOccupied(x: number, y: number, w: number, h: number): boolean {
    const x1 = Math.floor(x / this.cellSize), y1 = Math.floor(y / this.cellSize);
    const x2 = Math.floor((x + w) / this.cellSize), y2 = Math.floor((y + h) / this.cellSize);
    for (let cx = x1; cx <= x2; cx++) for (let cy = y1; cy <= y2; cy++) if (this.cells.has(`${cx},${cy}`)) return true;
    return false;
  }
  occupy(x: number, y: number, w: number, h: number): void {
    const x1 = Math.floor(x / this.cellSize), y1 = Math.floor(y / this.cellSize);
    const x2 = Math.floor((x + w) / this.cellSize), y2 = Math.floor((y + h) / this.cellSize);
    for (let cx = x1; cx <= x2; cx++) for (let cy = y1; cy <= y2; cy++) this.cells.add(`${cx},${cy}`);
  }
}

// ─── PATH UTILITIES ──────────────────────────────────────────────────────────

function pDist(a: { x: number; y: number }, b: { x: number; y: number }): number { return Math.sqrt((a.x - b.x) ** 2 + (a.y - b.y) ** 2); }
function measurePath(path: { x: number; y: number }[]): number { let len = 0; for (let i = 1; i < path.length; i++) len += pDist(path[i - 1], path[i]); return len; }

function getPointAtDistance(path: { x: number; y: number }[], dist: number): { x: number; y: number; angle: number } {
  ...
  for (let i = 1; i < path.length; i++) {
    ...
    if (remaining <= segLen || i === path.length - 1) {
      ...
      return { x: path[i - 1].x + t * (path[i].x - path[i - 1].x), y: path[i - 1].y + t * (path[i].y - path[i - 1].y),
...
function getSubPath(path: { x: number; y: number }[], startDist: number, endDist: number): { x: number; y: number }[] {
  const result: { x: number; y: number }[] = []; let dist = 0; let started = false;
  for (let i = 0; i < path.length; i++) {
    if (i > 0) {
      const segLen = pDist(path[i - 1], path[i]); const newDist = dist + segLen;
      if (!started && newDist >= startDist) {
        const t = segLen > 0 ? (startDist - dist) / segLen : 0;
        result.push({ x: path[i - 1].x + t * (path[i].x - path[i - 1].x), y: path[i - 1].y + t * (path[i].y - path[i - 1].y) }); started = true;
      }
      if (started) {
        if (newDist >= endDist) {
          const t = segLen > 0 ? (endDist - dist) / segLen : 0;
          result.push({ x: path[i - 1].x + t * (path[i].x - path[i - 1].x), y: path[i - 1].y + t * (path[i].y - path[i - 1].y) }); break;
        }
        result.push({ x: path[i].x, y: path[i].y });
      }
      dist = newDist;
    } else if (startDist === 0) { result.push({ x: path[i].x, y: path[i].y }); started = true; }
  }
  return result;
}
...
function ensureLeftToRight(path: { x: number; y: number }[]): { x: number; y: number }[] {
  ...
  let sumDx = 0, sumDy = 0;
  for (let i = 1; i < path.length; i++) { sumDx += path[i].x - path[i - 1].x; sumDy += path[i].y - path[i - 1].y; }
  if (Math.abs(sumDx) >= Math.abs(sumDy)) return sumDx >= 0 ? path : [...path].reverse();
  return sumDy >= 0 ? path : [...path].reverse();
}

function mergePixelSegments(segments: { x: number; y: number }[][], threshold: number): { x: number; y: number }[][] {
  if (segments.length === 0) return [];
  const merged: { x: number; y: number }[][] = []; const used = new Set<number>();
  for (let i = 0; i < segments.length; i++) {
    if (used.has(i)) continue; used.add(i); let current = [...segments[i]]; let changed = true;
    while (changed) { changed = false;
      for (let j = 0; j < segments.length; j++) {
        if (used.has(j)) continue; const seg = segments[j];
        const cEnd = current[current.length - 1], cStart = current[0];
        if (pDist(cEnd, seg[0]) < threshold) { current = [...current, ...seg.slice(1)]; used.add(j); changed = true; }
        else if (pDist(cEnd, seg[seg.length - 1]) < threshold) { current = [...current, ...[...seg].reverse().slice(1)]; used.add(j); changed = true; }
        else if (pDist(cStart, seg[seg.length - 1]) < threshold) { current = [...seg, ...current.slice(1)]; used.add(j); changed = true; }
        else if (pDist(cStart, seg[0]) < threshold) { current = [...[...seg].reverse(), ...current.slice(1)]; used.add(j); changed = true; }
      }
    }
    merged.push(current);
  }
...
function queryAndGroupRoads(map: mapboxgl.Map): RoadPath[] {
  ...
  map.getStyle().layers?.forEach((layer: any) => {
    if (layer.type !== 'line') return; const id = layer.id.toLowerCase();
    if (id.startsWith('mm-') || id.startsWith('pcl-')) return;
    if (id.includes('road') || id.includes('street') || id.includes('bridge') || id.includes('tunnel')) roadLayerIds.push(layer.id);
  });
  ...
  for (const [name, data] of byName) {
    const pixelSegments = data.coords.map((coords) => coords.map(([lng, lat]) => { const p = map.project([lng, lat]); return { x: p.x, y: p.y }; }));
    const deduped: { x: number; y: number }[][] = [];
    for (const seg of pixelSegments) {
      if (seg.length < 2) continue;
      const isDup = deduped.some((ex) => { if (ex.length < 2) return false;
        return (pDist(seg[0], ex[0]) < 3 && pDist(seg[seg.length - 1], ex[ex.length - 1]) < 3) ||
               (pDist(seg[0], ex[ex.length - 1]) < 3 && pDist(seg[seg.length - 1], ex[0]) < 3); });
      if (!isDup) deduped.push(seg);
    }
    const merged = mergePixelSegments(deduped, 50);
    for (const path of merged) { const pl = measurePath(path); if (pl < 10) continue;
      results.push({ name, roadClass: data.roadClass, pixelPath: ensureLeftToRight(path), pathLength: pl }); }
  }
...
function drawTextAlongPath(ctx: CanvasRenderingContext2D, text: string, path: { x: number; y: number }[], fontSize: number, grid: OccupancyGrid): boolean {
  ctx.font = `bold ${fontSize}px Arial, Helvetica, sans-serif`; ctx.textBaseline = 'middle'; ctx.textAlign = 'center';
  const chars = [...text]; const charWidths = chars.map((c) => ctx.measureText(c).width * CONDENSE);
  const totalWidth = charWidths.reduce((a, b) => a + b, 0); const pathLen = measurePath(path);
  if (totalWidth > pathLen * 1.2) return false;
  const startDist = Math.max(0, (pathLen - totalWidth) / 2);
  const positions: { x: number; y: number; angle: number; w: number }[] = []; let dist = startDist;
  for (let i = 0; i < chars.length; i++) { const pos = getPointAtDistance(path, dist + charWidths[i] / 2); positions.push({ ...pos, w: charWidths[i] }); dist += charWidths[i]; }
  const pad = 6;
  for (const cp of positions) { if (grid.isOccupied(cp.x - cp.w / 2 - pad, cp.y - fontSize / 2 - pad, cp.w + pad * 2, fontSize + pad * 2)) return false; }
  for (let i = 0; i < chars.length; i++) {
    const cp = positions[i]; ctx.save(); ctx.translate(cp.x, cp.y);
    let angle = cp.angle; if (angle > Math.PI / 2) angle -= Math.PI; if (angle < -Math.PI / 2) angle += Math.PI;
    ctx.rotate(angle); ctx.scale(CONDENSE, 1.0);
    ctx.strokeStyle = '#ffffff'; ctx.lineWidth = Math.max(4, fontSize * 0.18); ctx.lineJoin = 'round'; ctx.strokeText(chars[i], 0, 0);
    ctx.fillStyle = '#111111'; ctx.fillText(chars[i], 0, 0); ctx.restore();
    grid.occupy(cp.x - cp.w / 2 - pad, cp.y - fontSize / 2 - pad, cp.w + pad * 2, fontSize + pad * 2);
  }
  return true;
}

function drawOffsetLabel(ctx: CanvasRenderingContext2D, text: string, path: { x: number; y: number }[], fontSize: number, grid: OccupancyGrid, offset: number): boolean {
  const pathLen = measurePath(path); const mid = getPointAtDistance(path, pathLen / 2);
  let angle = mid.angle; if (angle > Math.PI / 2) angle -= Math.PI; if (angle < -Math.PI / 2) angle += Math.PI;
  ctx.font = `bold ${fontSize}px Arial, Helvetica, sans-serif`; const tw = ctx.measureText(text).width * CONDENSE; const pad = 6;
  for (const sign of [1, -1]) {
    const perpAngle = mid.angle + (Math.PI / 2) * sign;
    const ox = mid.x + Math.cos(perpAngle) * offset, oy = mid.y + Math.sin(perpAngle) * offset;
    if (grid.isOccupied(ox - tw / 2 - pad, oy - fontSize / 2 - pad, tw + pad * 2, fontSize + pad * 2)) continue;
    ctx.save(); ctx.translate(ox, oy); ctx.rotate(angle); ctx.scale(CONDENSE, 1.0);
    ctx.textBaseline = 'middle'; ctx.textAlign = 'center';
    ctx.strokeStyle = '#ffffff'; ctx.lineWidth = Math.max(4, fontSize * 0.18); ctx.lineJoin = 'round'; ctx.strokeText(text, 0, 0);
    ctx.fillStyle = '#333333'; ctx.fillText(text, 0, 0); ctx.restore();
    grid.occupy(ox - tw / 2 - pad, oy - fontSize / 2 - pad, tw + pad * 2, fontSize + pad * 2); return true;
  }
  return false;
}

function drawLegendMarker(ctx: CanvasRenderingContext2D, num: number, x: number, y: number, grid: OccupancyGrid): boolean {
  const r = 22;
  for (const [dx, dy] of [[0, 0], [0, -28], [0, 28], [-28, 0], [28, 0]]) {
    ...
    ctx.font = `bold 18px Arial, sans-serif`; ctx.textBaseline = 'middle'; ctx.textAlign = 'center';
    ctx.fillStyle = '#000000'; ctx.fillText(String(num), px, py);
    grid.occupy(px - r, py - r, r * 2, r * 2); return true;
  }
  return false;
}

function drawStraightHorizontalLabel(ctx: CanvasRenderingContext2D, text: string, x: number, y: number, fontSize: number, grid: OccupancyGrid): boolean {
  ctx.font = `bold ${fontSize}px Arial, Helvetica, sans-serif`; const tw = ctx.measureText(text).width * CONDENSE; const pad = 6;
  if (grid.isOccupied(x - tw / 2 - pad, y - fontSize / 2 - pad, tw + pad * 2, fontSize + pad * 2)) return false;
  ctx.save(); ctx.translate(x, y); ctx.scale(CONDENSE, 1.0); ctx.textBaseline = 'middle'; ctx.textAlign = 'center';
  ctx.strokeStyle = '#ffffff'; ctx.lineWidth = Math.max(5, fontSize * 0.18); ctx.lineJoin = 'round'; ctx.strokeText(text, 0, 0);
  ctx.fillStyle = '#111111'; ctx.fillText(text, 0, 0); ctx.restore();
  grid.occupy(x - tw / 2 - pad, y - fontSize / 2 - pad, tw + pad * 2, fontSize + pad * 2); return true;
}

// ─── 3-PASS LABELING ─────────────────────────────────────────────────────────

function labelRoadsThreePass(ctx: CanvasRenderingContext2D, roads: RoadPath[]): { entries: LegendEntry[]; grid: OccupancyGrid } {
  const grid = new OccupancyGrid(14); const legendEntries: LegendEntry[] = []; let legendNum = 1;
  const labeledNames = new Map<string, number>(); const labelPositions = new Map<string, { x: number; y: number }[]>();
  const REPEAT_INTERVAL = 600; const MIN_SAME_NAME_DIST = 400;
  for (const road of roads) {
    const fontSize = getFontSize(road.roadClass); const currentCount = labeledNames.get(road.name) || 0;
    ...
    const numPlacements = Math.min(maxLabels - currentCount, Math.max(1, Math.floor(road.pathLength / REPEAT_INTERVAL)));
    for (let p = 0; p < numPlacements; p++) {
      ...
      if (existingPositions.some((pos) => pDist(pos, midPt) < MIN_SAME_NAME_DIST)) continue;
      ...
      if (isMajorHighway) {
        if (drawStraightHorizontalLabel(ctx, road.name, midPt.x, midPt.y, fontSize, grid)) {
          ...
          labelPositions.get(road.name)!.push(midPt); continue;
        }
      }
      ...
      if (!labeledNames.has(road.name)) {
        ...
        if (drawLegendMarker(ctx, legendNum, mid.x, mid.y, grid)) {
          ...
          legendNum++; labeledNames.set(road.name, 1);
        }
      }
    }
  }
...
// ─── MARK COLORED PIXELS (ROUTES/DOTS) AS OCCUPIED ──────────────────────────

function markColoredPixelsOccupied(ctx: CanvasRenderingContext2D, grid: OccupancyGrid, canvasW: number, canvasH: number): void {
  const imageData = ctx.getImageData(0, 0, canvasW, canvasH);
  const px = imageData.data;
  const blockSize = 28;
  const buffer = 14;
  const sampleStep = Math.max(1, Math.floor(blockSize / 4));
  for (let by = 0; by < canvasH; by += blockSize) {
    for (let bx = 0; bx < canvasW; bx += blockSize) {
      let hasColor = false;
      for (let sy = by; sy < Math.min(by + blockSize, canvasH) && !hasColor; sy += sampleStep) {
        for (let sx = bx; sx < Math.min(bx + blockSize, canvasW) && !hasColor; sx += sampleStep) {
          const i = (sy * canvasW + sx) * 4;
          const r = px[i], g = px[i + 1], b = px[i + 2];
          const mx = Math.max(r, g, b), mn = Math.min(r, g, b);
          const sat = mx > 0 ? (mx - mn) / mx : 0;
          if (sat > 0.20 && mx > 60) hasColor = true;
        }
      }
      if (hasColor) grid.occupy(bx - buffer, by - buffer, blockSize + buffer * 2, blockSize + buffer * 2);
    }
  }
}

// ─── LEGEND — WIDE, SHORT, BOTTOM-FIRST, ROUTE-AWARE ────────────────────────

function drawLegendOnCanvas(ctx: CanvasRenderingContext2D, grid: OccupancyGrid, entries: LegendEntry[], canvasW: number, canvasH: number): void {
  ...
  const targetRowsPerCol = 12;
  const COLS = Math.max(2, Math.min(6, Math.ceil(entries.length / targetRowsPerCol)));
  const rows = Math.ceil(entries.length / COLS);
  const boxW = Math.min(COLS * colW + pad * 2, Math.round(canvasW * 0.65));
  const boxH = rows * rowH + pad * 2;

  // Bottom-first candidates
  const candidates = [
  ...
  for (const pos of candidates) {
    if (!grid.isOccupied(pos.x, pos.y, boxW, boxH)) { bestPos = pos; break; }
  }
  ...
  ctx.font = `${fontSize}px Arial, sans-serif`; ctx.textBaseline = 'top'; ctx.textAlign = 'left'; ctx.fillStyle = '#000000';
  for (let i = 0; i < entries.length; i++) {
    const col = Math.floor(i / rows), row = i % rows;
    const ex = bestPos.x + pad + col * colW, ey = bestPos.y + pad + row * rowH;
    ctx.fillText(`${entries[i].number}. ${entries[i].streetName}`, ex, ey);
  }
  grid.occupy(bestPos.x, bestPos.y, boxW, boxH);
}

// ─── COMPOSITE: greyscale + labels + route-aware legend ─────────────────────

async function processMapWithLabels(rawDataUrl: string, roads: RoadPath[], width: number, height: number): Promise<MastermapRenderResult> {
  return new Promise((resolve) => {
    const img = new Image();
    img.onload = () => {
      ...
      for (let i = 0; i < px.length; i += 4) {
        ...
        const mx = Math.max(r, g, b), mn = Math.min(r, g, b);
        const sat = mx > 0 ? (mx - mn) / mx : 0;
        if (sat < 0.18) { const grey = Math.round(0.299 * r + 0.587 * g + 0.114 * b); px[i] = grey; px[i + 1] = grey; px[i + 2] = grey; }
      }
      ctx.putImageData(imageData, 0, 0);

      // Draw road labels (3-pass: inline → offset → numbered marker)
      const { entries: legendEntries, grid } = labelRoadsThreePass(ctx, roads);

      // Mark route lines and booking dots as occupied so legend avoids them
      markColoredPixelsOccupied(ctx, grid, width, height);

      // Draw legend — wide, short, bottom-first
      drawLegendOnCanvas(ctx, grid, legendEntries, width, height);

      resolve({ imageDataUrl: canvas.toDataURL('image/png'), legendEntries });
    };
...
// ─── OFF-SCREEN MASTERMAP RENDERER ───────────────────────────────────────────

async function renderMastermapOffscreen(routes: SavedRouteInput[], bookings: MastermapBookingInput[], pixelW: number, pixelH: number): Promise<MastermapRenderResult | null> {
  ...
  // Filter outlier coordinates before computing bounds
  allCoords = filterOutlierCoords(allCoords);
  ...
  return new Promise<MastermapRenderResult | null>((resolve) => {
    ...
    document.body.appendChild(container);
    let done = false;
    const cleanup = () => { try { document.body.removeChild(container); } catch { /* ok */ } };

    mapboxgl.accessToken = (import.meta.env.VITE_MAPBOX_TOKEN as string) || '';
    const map = new mapboxgl.Map({
      container, style: 'mapbox://styles/mapbox/streets-v12', center: allCoords[0],
      zoom: 12, preserveDrawingBuffer: true, attributionControl: false, interactive: false,
    });
    ...
    const timeout = setTimeout(() => {
      ...
      try { map.triggerRepaint();
        setTimeout(() => { if (done) return; done = true;
          try { const url = map.getCanvas().toDataURL('image/png'); const roads = queryAndGroupRoads(map);
            try { map.remove(); } catch {} cleanup(); processMapWithLabels(url, roads, pixelW, pixelH).then(resolve);
    ...
    map.on('load', () => {
      applyMapStyling(map); applyGreyscaleBase(map); thickenMastermapRoads(map); hideMapboxLabels(map);

      const routeInsertBefore = map.getStyle().layers?.find((l: any) => l.type === 'symbol' && l.id.startsWith('mm-'))?.id ?? undefined;

      routes.forEach((route, idx) => {
        if (!route.segments?.length) return;
        map.addSource(`mm-route-${idx}`, { type: 'geojson', data: { type: 'FeatureCollection',
          features: route.segments.map((s) => ({ type: 'Feature' as const, properties: {}, geometry: { type: 'LineString' as const, coordinates: s.coordinates } })) } });
        map.addLayer({ id: `mm-line-${idx}`, type: 'line', source: `mm-route-${idx}`,
          paint: { 'line-color': route.route_color, 'line-width': 28, 'line-opacity': 0.85 },
          layout: { 'line-cap': 'round', 'line-join': 'round' } }, routeInsertBefore);
      });

      const labelFeatures: GeoJSON.Feature[] = [];
      routes.forEach((route) => {
        const coords: [number, number][] = []; route.segments?.forEach((s) => coords.push(...s.coordinates));
        if (!coords.length) return;
        const cLng = coords.reduce((sum, c) => sum + c[0], 0) / coords.length;
        const cLat = coords.reduce((sum, c) => sum + c[1], 0) / coords.length;
        labelFeatures.push({ type: 'Feature', properties: { num: String(route.route_number), color: route.route_color },
      ...
      if (labelFeatures.length) {
        ...
        map.addLayer({ id: 'mm-route-nums', type: 'symbol', source: 'mm-labels',
      ...
      if (bookings.length) {
        map.addSource('mm-bookings', { type: 'geojson', data: { type: 'FeatureCollection',
          features: bookings.map((b) => ({ type: 'Feature' as const, properties: { routeColor: b.routeColor },
            geometry: { type: 'Point' as const, coordinates: [b.lng, b.lat] } })) } });
        map.addLayer({ id: 'mm-booking-dots', type: 'circle', source: 'mm-bookings',
          ...
          setTimeout(() => {
            ...
            map.once('render', () => {
              setTimeout(() => {
                ...
                } catch (err) { console.error('[DMB Mastermap] capture failed:', err); try { map.remove(); } catch {} cleanup(); resolve(null); }
              }, 300);
            });
          }, 2000);
        });
...
export async function generateMastermap(
  areaName: string, routes: SavedRouteInput[], geocodedBookings: MastermapBookingInput[],
  bookingsData: Map<string, any[]>, onProgress?: (p: DmbPCLProgress) => void,
  pageFormat: MastermapPageFormat = 'letter-portrait',
): Promise<MastermapResult> {
  if (!routes.length) return { success: false, routeCount: 0, bookingCount: 0, errorMessage: 'No routes in this area.' };
  ...
  const mapX = MM_MARGIN; const mapY = MM_MARGIN + sidebarH;
  const mapAreaW = pageW - 2 * MM_MARGIN; const mapAreaH = pageH - 2 * MM_MARGIN - sidebarH;
  const pixelW = 5000; const pixelH = Math.round(pixelW * (mapAreaH / mapAreaW));

  onProgress?.({ phase: 'Rendering map', detail: 'Capturing + labeling streets\u2026', percent: 10 });
  await yieldUI(); await new Promise((r) => setTimeout(r, 1000));
  ...
  if (renderResult?.imageDataUrl) {
    try { doc.addImage(renderResult.imageDataUrl, 'PNG', mapX, mapY, mapAreaW, mapAreaH); } catch { /* map unavailable */ }
  }

  const sortedRoutes = [...routes].sort((a, b) => a.route_number - b.route_number);
  function hexToRgb(hex: string) { const h = hex.replace('#', ''); return { r: parseInt(h.slice(0, 2), 16) || 0, g: parseInt(h.slice(2, 4), 16) || 0, b: parseInt(h.slice(4, 6), 16) || 0 }; }

  const sy = MM_MARGIN; let sx = MM_MARGIN;
  doc.setFont('helvetica', 'bold'); doc.setFontSize(16); doc.setTextColor(0, 0, 0);
  doc.text(areaName, sx, sy + 14); sx += areaName.length * 9.5 + 12;
  doc.setFontSize(12); let rowY = sy;
  for (const route of sortedRoutes) {
    const count = bookingsData.get(route.route_code)?.length ?? 0;
    const label = `${route.route_code}(${count})`; const entryW = label.length * 7 + 18;
    if (sx + entryW > pageW - MM_MARGIN) { sx = MM_MARGIN; rowY += 20; if (rowY > MM_MARGIN + sidebarH - 12) break; }
    const rgb = hexToRgb(route.route_color);
    doc.setFillColor(rgb.r, rgb.g, rgb.b); doc.circle(sx + 4, rowY + 11, 4, 'F');
    doc.setFont('helvetica', 'bold'); doc.setTextColor(30, 30, 30); doc.text(route.route_code, sx + 12, rowY + 14);
    doc.setFont('helvetica', 'normal'); doc.setTextColor(100, 100, 100);
    doc.text(`(${count})`, sx + 12 + route.route_code.length * 7 + 1, rowY + 14); sx += entryW;
  }
  doc.setDrawColor(180, 180, 180); doc.setLineWidth(0.5);
  doc.line(MM_MARGIN, mapY - 3, pageW - MM_MARGIN, mapY - 3);
  doc.setFontSize(3.5); doc.setTextColor(160, 160, 160);
