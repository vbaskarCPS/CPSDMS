...
export const parseDailySessionXLSX = async (
  file: File
  ...
  if (!routesSheet || !workersSheet || !bookingsSheet) {
    throw new Error(
      "Invalid Data Feed. Missing required tabs: 'Routes', 'Workers', or 'Bookings'."
    );
  }
    ...
    if (managerName && routeCode) {
      if (!managersMap.has(managerName)) {
        ...
        managersMap.set(managerName, {
          userId: generateConsistentId(managerName, 'rm'), // FIXED ID
          name: managerName,
          username,
          password,
          role: 'RouteManager',
  ...
  // --- PROCESS BOOKINGS ---
  const pendingBookings: MasterBooking[] = bookingsData
    .map((row: any, index: number) => {
      const routeNum = row['Route #'];
      if (!routeNum) return null;

      return {
        'Booking ID': `job_${index}_${Date.now()}`, // Job IDs can remain random/timestamped as they are transient per day
        'Route Number': routeNum,
        'First Name': row['First Name'],
        'Last Name': row['Last Name'],
        'House Number': row['House #'],
        'Street Name': row['Street Name'],
        'Full Address': `${row['House #'] || ''} ${
          row['Street Name'] || ''
        }`.trim(),
